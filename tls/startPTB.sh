...
# This script requires a pre configured docker environment, with the four
...
docker exec peer0.$DOMAIN peer channel create -o $ORDERER -c $CHANNEL -f /etc/hyperledger/configtx/$CHANNEL.tx --tls true --cafile /etc/hyperledger/tlscacerts/tlsca.ptb.de-cert.pem

echo Join peer0.org1.example.com to the channel.
docker exec peer0.$DOMAIN peer channel join -b $CHANNEL.block

echo Fetch the channel in any one of the other peers
docker exec peer1.$DOMAIN peer channel fetch config -o $ORDERER -c $CHANNEL --tls --cafile /etc/hyperledger/tlscacerts/tlsca.ptb.de-cert.pem
...
echo Join each one of the peers to the channel.
